...
import { DataGridCellWrapper } from "@/registry/bases/aria/components/data-grid/data-grid-cell-wrapper";
...
import { Popover } from "@/registry/bases/aria/ui/popover";
import {
  Select,
  SelectContent,
  SelectGroup,
...
export function LongTextCell<TData extends RowData>({
  ...
  React.useEffect(() => {
    if (isEditing) onInitialFocus();
  }, [isEditing, onInitialFocus]);
      ...
      {isEditing ? (
        ...
        >
          ...
          <SelectContent
            ...
            crossOffset={-8}
            offset={-8}
            className="min-w-[calc(var(--trigger-width)+16px)]"
          >
            <SelectGroup>
              {options.map((option) => (
                ...
                {options.map((option) => {
                  ...
                  return (
                    <CommandItem
                      ...
                      onAction={() => onValueChange(option.value)}
                    >
                      <div
                        className={cn(
      ...
      {isEditing && (
        ...
        >
          <Calendar
            ...
            defaultFocusedValue={
              selectedDate
                ? toCalendarDate(fromDateToLocal(selectedDate))
            ...
            onChange={(date) => onDateSelect(date.toDate(getLocalTimeZone()))}
          />
        </Popover>
      )}
    </DataGridCellWrapper>
